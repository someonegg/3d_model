...
from tools.model_library.resources import builder_path, build_order, dependencies, discover, resource, input_hashes
from tools.model_library.validation import current_report, inspect
from tests.support import write_manifest, seed_validation_report, write_box_model
...
class ModelTests(unittest.TestCase):
    ...
    def test_stale_report(self):
        model = write_box_model(self.root)
...
''')
    ...
    def test_invalid_reports_leave_catalog_available(self):
        model = write_box_model(self.root)
        for invalid in ([], [{'file': 'mesh.stl'}], {'triangles': 12}, None):
            with self.subTest(report=invalid):
                (self.root / 'validation.json').write_text(json.dumps(invalid))
                write_manifest(self.root, model)
    ...
    def test_discovery_ignores_test_and_hidden_directories(self):
        ...
        for name in ('models/.hidden', 'tests', 'tmp'):
            directory = self.library / name
            directory.mkdir()
            (directory / 'model.json').write_text('{}')
        self.assertEqual([model['id'] for _, model in discover(self.library)], ['example'])

    def test_nested_scripts_and_shared_inputs_stay_within_library(self):
        source = self.root / 'src'
        source.mkdir()
        (source / 'build.py').write_text('pass')
        shared = self.library / 'tools'
        shared.mkdir()
        (shared / 'shared.py').write_text('value = 1')
        model = write_manifest(self.root)
        model.update(build='src/build.py', inputs=['src/build.py', '../../tools/shared.py'])
        write_manifest(self.root, model)
        self.assertEqual(builder_path(self.root, model), source / 'build.py')
        self.assertEqual(len(input_hashes(self.root, model)), 2)
        self.assertEqual(discover(self.library)[0][1], model)
        outside = self.library.parent / 'outside-input.py'
        outside.write_text('outside')
        self.addCleanup(outside.unlink)
        with self.assertRaisesRegex(ValueError, '越界'):
            input_hashes(self.root, model | {'inputs': ['../../../outside-input.py']})
        (shared / 'escape.py').symlink_to(outside)
        with self.assertRaisesRegex(ValueError, '越界'):
            input_hashes(self.root, model | {'inputs': ['../../tools/escape.py']})

    def test_build_order_follows_inputs_and_rejects_cycles(self):
        first = self.library / 'models/a-display'
        last = self.library / 'models/z-print'
        a = {'id': first.name, 'inputs': ['../z-print/assembled.glb']}
        z = {'id': last.name, 'inputs': ['src/build.py']}
        self.assertEqual(build_order([(first, a), (last, z)]), [(last, z), (first, a)])
        self.assertEqual(build_order([(first, a)]), [(first, a)])
        z['inputs'] = ['../a-display/preview.png']
        with self.assertRaisesRegex(ValueError, '依赖循环'):
            build_order([(first, a), (last, z)])
    ...
    def test_cli_uses_explicit_model_root_and_output(self):
        model = write_box_model(self.root)
        cli = Path(__file__).resolve().parents[2] / 'tools/models.py'
        def run(*args):
            return subprocess.run([sys.executable, str(cli), *args, '--model-root', str(self.library)],
    ...
    def test_cli_default_site_paths_ignore_model_root(self):
        model = write_box_model(self.root)
        validate(self.root, model)
        cli = Path(__file__).resolve().parents[2] / 'tools/models.py'
        with patch.object(sys, 'path', [str(cli.parent), *sys.path]):
            main = runpy.run_path(str(cli))['main']
        repository = self.library / 'source-repository'
        with patch.dict(main.__globals__, REPO_ROOT=repository):
            for command in ('site', 'publish'):
                with patch.object(sys, 'argv', [str(cli), command, '--model-root', str(self.library)]):
                    self.assertEqual(main(), 0)
                if command == 'site':
                    staged = repository / 'tmp/site'
                    self.assertTrue((staged / 'catalog.json').is_file())
                    (staged / 'index.html').write_text('fixture')
        self.assertFalse(staged.exists())
        index = json.loads((repository / 'dist/catalog.json').read_text())
        self.assertEqual([entry['id'] for entry in index], ['example'])
        self.assertFalse((self.library / 'tmp/site').exists())
        self.assertFalse((self.library / 'dist').exists())
    ...
    def test_copy_and_publish_failures_preserve_model_and_site(self):
        model = write_box_model(self.root, readme='original')
