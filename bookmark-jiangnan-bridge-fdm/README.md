...
- `source.png`：生成高度层级的源图。
- `parameters.json`：控制 0～10 层白色覆盖的灰度映射。
...
源图与提示词见 `source.png`、`source-prompt.md`，共享书签几何与灰度映射位于 `tools/bookmark_relief.py`。脚本支持 `MODEL_OUTPUT_DIR`，无需 HueForge；构建生成 `bookmark-jiangnan-bridge-fdm.stl` 和正式 `preview.png`，随后对最终 STL 执行通用几何校验。尺寸和层高受脚本约束，修改时需同步调整说明。
