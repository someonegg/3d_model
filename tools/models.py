...
from model_library.resources import discover, build_order
...
def main():
    ...
    parser.add_argument('--model-root', type=Path, default=REPO_ROOT, help='model library directory (default: source repository)')
    parser.add_argument('--output', type=Path, help='site or publish output directory')
    parser.add_argument('--staging', type=Path, help='publish staging directory (default: <repository>/tmp/site)')
    ...
    try:
        if args.command == 'site':
            site(model_root, (args.output or REPO_ROOT / 'tmp/site').resolve())
        elif args.command == 'publish':
            publish_site((args.staging or REPO_ROOT / 'tmp/site').resolve(),
                         (args.output or REPO_ROOT / 'dist').resolve())
        elif args.command == 'catalog':
            print(json.dumps(catalog(model_root), ensure_ascii=False))
        else:
            selected = [(d, m) for d, m in discover(model_root) if args.all or m['id'] == args.id]
            if not selected:
                raise ValueError('请指定有效模型 ID 或 --all')
            action = {'build': build, 'validate': validate}[args.command]
            if args.command == 'build':
                selected = build_order(selected)
