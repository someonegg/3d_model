...
import { repoRoot, runPython } from "./runtime.mjs";

const excluded = new Set(
  JSON.parse(
    readFileSync(path.join(repoRoot, "tools/model-discovery.json"), "utf8"),
  ).excluded_directories,
);
const ignored = (name) => name.startsWith(".") || excluded.has(name);
    ...
    configureServer(server) {
            ...
            inputFiles = new Set(
              models.flatMap((model) =>
                (model.inputs ?? []).map((name) =>
                  path.resolve(root, model.id, name),
                ),
              ),
            );
      ...
      const watchModels = async () => {
        for (const entry of await readdir(root, { withFileTypes: true })) {
          if (entry.isDirectory() && !ignored(entry.name))
            server.watcher.add(path.join(root, entry.name));
        }
      };
      void watchModels();
      const rootWatcher = watch(root, () => {
        void watchModels();
      });
      ...
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          const file = await realpath(path.resolve(root, id, name));
          if (!file.startsWith(path.join(root, id) + path.sep))
            throw Error("无效路径");
