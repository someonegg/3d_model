...
def discover(root):
    models = []
    for path in sorted(root.glob('models/*/model.json')):
...
def build_order(models):
    """Order selected models by cross-model inputs without rebuilding unselected ones."""
    pending = {directory.resolve(): (directory, model) for directory, model in models}
    ordered, active, done = [], set(), set()

    def visit(directory):
        if directory in active:
            raise ValueError(f'模型构建依赖循环：{directory.name}')
        if directory in done:
            return
        active.add(directory)
        original, model = pending[directory]
        for name in model.get('inputs', []):
            source = (directory / name).resolve()
            for dependency in pending:
                if dependency != directory and source.is_relative_to(dependency):
                    visit(dependency)
        active.remove(directory)
        done.add(directory)
        ordered.append((original, model))
